...
public class Client : MonoBehaviour
{
    ...
    void PredictLowerPose()
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            Body_Joints[i].transform.localRotation = Quaternion.LookRotation(forward, upward);
        }
    ...
    void PlayAnimation()
    {
        ...
        if (!calibrated)
        {
            for (int j = 0; j < Body_Joints.Length; j++)
            {
                ...
                Body_Joints[j].transform.localRotation = Quaternion.LookRotation(forward, up);
            }
        }
    }
}
